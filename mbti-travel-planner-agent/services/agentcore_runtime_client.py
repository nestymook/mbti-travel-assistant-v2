...
class AgentCoreRuntimeClient:
    ...
    def __init__(
        ...
        if self.enable_connection_pooling:
            try:
                from .connection_pool_manager import get_connection_pool_manager, ConnectionConfig
                if pool_config is None:
                    pool_config = ConnectionConfig()  # Use default configuration
    ...
    def _init_aws_client(self):
        ...
        logger.debug("AWS Bedrock Agent Runtime client initialized")
        ...
        try:
            ...
            async def protected_call():
                if self.enable_connection_pooling and self.connection_pool_manager:
                    ...
                    async with self.connection_pool_manager.get_client(
                        'bedrock-agent-runtime', self.region
                    ) as client:
                        return await self._invoke_agent_with_client(client, agent_arn, input_text, session_id)
                else:
                    # Use direct client
    ...
    async def _invoke_agent_direct(
        ...
        return await self._invoke_agent_with_client(
            self.bedrock_agent_runtime, agent_arn, input_text, session_id
        )
    ...
    async def _invoke_agent_with_client(
        ...
        try:
            ...
            agent_id = self._extract_agent_id(agent_arn)
    ...
    def _extract_agent_id(self, agent_arn: str) -> str:
        """Extract agent ID from ARN."""
        try:
            # ARN format: arn:aws:bedrock-agentcore:region:account:runtime/agent-name-id
            if agent_arn.startswith('arn:aws:bedrock-agentcore:'):
                parts = agent_arn.split('/')
                if len(parts) >= 2:
                    full_agent_name = parts[-1]  # Get the last part after the final slash
                    # Extract just the agent ID part (before the dash and random suffix)
                    # Format is usually: agent-name-randomId
                    # For bedrock-agent-runtime, we need just the base name
                    if '-' in full_agent_name:
                        # Take the first part before the first dash as the agent ID
                        base_name = full_agent_name.split('-')[0]
                        # But if it's too short, use more parts
                        if len(base_name) < 3:
                            parts_list = full_agent_name.split('-')
                            if len(parts_list) >= 2:
                                base_name = '-'.join(parts_list[:2])
                        return base_name[:10]  # Limit to 10 characters as per AWS constraint
                    else:
                        return full_agent_name[:10]
                else:
                    # If no slash, assume the entire string is the agent ID
                    return agent_arn.split(':')[-1][:10]
            else:
                # Not an ARN, assume it's already an agent ID
                return agent_arn[:10]
        except Exception as e:
            raise ConfigurationError(f"Invalid agent ARN format: {agent_arn}") from e
    ...
    async def invoke_agent_with_streaming(
        ...
        Yields:
            AgentStreamingResponse objects
        """
        if not session_id:
            session_id = str(uuid.uuid4())
        ...
        try:
            ...
            response = await asyncio.get_event_loop().run_in_executor(
                None,
                lambda: self.bedrock_agent_runtime.invoke_agent(
                    agentId=agent_id,
