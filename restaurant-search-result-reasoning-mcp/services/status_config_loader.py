"""
Configuration loader for reasoning MCP server status check system.

This module handles loading, validation, and management of status check
configurations from JSON files and environment variables for reasoning server.
"""
...
class StatusCheckConfigLoader:
    """Loads and manages status check configuration for reasoning server."""
    
    def __init__(self, config_file_path: str = "config/status_check_config.json"):
        """Initialize the configuration loader for reasoning server.
    ...
    def load_config(self, force_reload: bool = False) -> Dict[str, Any]:
        ...
        if not config_path.exists():
            raise FileNotFoundError(f"Reasoning status check config file not found: {config_path}")
        ...
        try:
            ...
            raise json.JSONDecodeError(
                f"Invalid JSON in reasoning status check config file: {e.msg}",
                e.doc,
                e.pos
            )
    ...
    def _apply_env_overrides(self) -> None:
        ...
        env_mappings = {
            'REASONING_MCP_STATUS_CHECK_ENABLED': ('status_check_system', 'enabled'),
            'REASONING_MCP_STATUS_CHECK_INTERVAL': ('status_check_system', 'global_check_interval_seconds'),
            'REASONING_MCP_STATUS_CHECK_TIMEOUT': ('status_check_system', 'global_timeout_seconds'),
            'REASONING_MCP_GATEWAY_BASE_URL': None,  # Special handling for server URLs
            'REASONING_JWT_TOKEN': None,  # Special handling for authentication
        }
        ...
        for env_var, config_path in env_mappings.items():
            ...
            if env_value is not None:
                if config_path is None:
                    # Special handling
                    if env_var == 'REASONING_MCP_GATEWAY_BASE_URL':
                        self._update_server_urls(env_value)
                    elif env_var == 'REASONING_JWT_TOKEN':
    ...
    def get_server_configs(self) -> Dict[str, MCPStatusCheckConfig]:
        """Get all reasoning server configurations as MCPStatusCheckConfig objects.
        ...
        for server_name, server_data in servers_config.items():
            ...
            # Get expected tools for reasoning server
            expected_tools = server_data.get('expected_tools', ["recommend_restaurants", "analyze_restaurant_sentiment"])
    ...
    def get_system_config(self) -> Dict[str, Any]:
        """Get system-level configuration settings for reasoning server.
    ...
    def get_authentication_config(self) -> Dict[str, Any]:
        """Get authentication configuration for reasoning server.
    ...
    def get_logging_config(self) -> Dict[str, Any]:
        """Get logging configuration for reasoning server.
    ...
    def get_alerting_config(self) -> Dict[str, Any]:
        """Get alerting configuration for reasoning server.
    ...
    def is_server_enabled(self, server_name: str) -> bool:
        """Check if a specific reasoning server is enabled for status checking.
    ...
    def get_enabled_servers(self) -> List[str]:
        """Get list of enabled reasoning server names.
        
        Returns:
            List of enabled reasoning server names
        """
        config_data = self.load_config()
        servers = config_data.get('servers', {})
    ...
    def validate_config(self) -> List[str]:
        """Validate the loaded configuration for reasoning server.
        ...
        try:
            ...
            return [f"Failed to load reasoning config: {str(e)}"]
        ...
        if not isinstance(servers, dict):
            ...
            for server_name, server_config in servers.items():
                if not isinstance(server_config, dict):
                    errors.append(f"Reasoning server config for '{server_name}' must be a dictionary")
                    continue
                ...
                for field in required_fields:
                    if field not in server_config:
                        errors.append(f"Reasoning server '{server_name}' missing required field: {field}")
                
                # Validate URL format
                endpoint_url = server_config.get('endpoint_url', '')
                if not endpoint_url.startswith(('http://', 'https://')):
                    errors.append(f"Reasoning server '{server_name}' endpoint_url must be a valid HTTP/HTTPS URL")
                
                # Validate expected tools for reasoning server
                expected_tools = server_config.get('expected_tools', [])
                if not isinstance(expected_tools, list):
                    errors.append(f"Reasoning server '{server_name}' expected_tools must be a list")
                else:
                    required_reasoning_tools = ["recommend_restaurants", "analyze_restaurant_sentiment"]
                    for tool in required_reasoning_tools:
                        if tool not in expected_tools:
                            errors.append(f"Reasoning server '{server_name}' missing required tool: {tool}")
                ...
                for field, (min_val, max_val) in numeric_fields.items():
                    if field in server_config:
                        ...
                        if not isinstance(value, (int, float)):
                            errors.append(f"Reasoning server '{server_name}' {field} must be numeric")
                        elif not (min_val <= value <= max_val):
                            errors.append(f"Reasoning server '{server_name}' {field} must be between {min_val} and {max_val}")
        
        return errors
    
    def save_config(self, config_data: Dict[str, Any]) -> None:
        """Save configuration to file for reasoning server.
        ...
        try:
            ...
            raise OSError(f"Failed to save reasoning config file: {str(e)}")
...
# Global configuration loader instance for reasoning server
_reasoning_config_loader: Optional[StatusCheckConfigLoader] = None


def get_config_loader(config_file_path: str = "config/status_check_config.json") -> StatusCheckConfigLoader:
    """Get the global configuration loader instance for reasoning server.
    
    Args:
        config_file_path: Path to configuration file
        
    Returns:
        StatusCheckConfigLoader instance for reasoning server
    """
    global _reasoning_config_loader
    if _reasoning_config_loader is None:
        _reasoning_config_loader = StatusCheckConfigLoader(config_file_path)
    return _reasoning_config_loader


def load_reasoning_server_configs() -> Dict[str, MCPStatusCheckConfig]:
    """Convenience function to load reasoning server configurations.
    
    Returns:
        Dictionary mapping reasoning server names to configuration objects
    """
    loader = get_config_loader()
    return loader.get_server_configs()


def validate_reasoning_status_check_config(config_file_path: str = "config/status_check_config.json") -> List[str]:
    """Validate reasoning status check configuration file.
